...
const artifact:Artifact={
 ...
 forecasts:[{date:'2011-01-03',predicted:120,lowerBound:90,upperBound:150}],anomalies:[],insights:[],modelCard:{}
}
test('dashboard totals reconcile and full view includes forecast',()=>{const result=dashboardPayload(artifact,{}) as any;assert.equal(result.kpis[0].value,250);assert.equal(result.kpis[1].value,5);assert.equal(result.kpis[2].value,1);assert.equal(result.forecasts.length,1)})
